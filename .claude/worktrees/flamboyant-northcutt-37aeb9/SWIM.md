...
Tested with all 5 feeds connected for ~6 minutes. All data below is real.

### Connection Status (`GET /api/swim/status`)

| Feed | Connected | Messages | Rate |
|------|-----------|----------|------|
| FNS | true | 38 | ~6/min |
| TFMS | true | 6,709 | ~1,100/min |
| SFDPS | true | 4,730 | ~780/min |
| ITWS | true | 8,854 | ~1,500/min |
| STDDS | true | 160,041 | ~26,600/min (!) |

**STDDS is by far the highest volume** — 160k messages in 6 minutes. Surface position updates from every monitored airport.

### Data Inventory After 6 Minutes

| Table | Rows | Notes |
|-------|------|-------|
| flight_plans | 16,752 | 16,484 with origin+dest, 535 currently ACTIVE |
| flow_events | 23,206 | 18,988 TMI lists, 3,381 restrictions, 406 FCAs, 384 advisories, 6 GDPs |
| airport_configs | 42 | Real-time runway/rate/weather for 42 airports |
| callsign_routes | 15,827 | 14,528 from TFMS (92%), 1,299 from ADSBdb (8%) |
| notams | 739 | 8 TFRs, 48 obstacles, 24 runway closures, 22 taxiway closures |
| terminal_weather | 31,897 | 6,919 tornado, 2,583 microburst, 2,202 windshear, 2,000 gust front |
| surface_events | 59,319 | 16,723 TAIS (TRACON tracks), 42,596 pending parse improvement |
| **DB size** | **58.8 MB** | After 6 minutes |

### Flight Plan Field Coverage

| Field | Count | Coverage | Example |
|-------|-------|----------|---------|
| acid (callsign) | 16,752 | 100% | `SWA1136`, `UAL649`, `FFT4989` |
| dep_arpt + arr_arpt | 16,484 | 98% | `KBNA→KSAN`, `KSFO→KTPA` |
| aircraft_type | 16,266 | 97% | `JET` (from TFMS `aircraftCategory`) |
| etd | 15,986 | 95% | `2026-04-05T04:20:00Z` (from TFMS `igtd`) |
| flight_status | 5,245 | 31% | `ACTIVE`, `PLANNED`, `COMPLETED` |
| altitude | 3,093 | 18% | `340`, `360C` (flight level, C=confirmed) |
| route | 2,301 | 14% | `KSFO./.EHF128063..BLH.J169.TFD.J50.ELP..KOBLE..DLF..KINEY..SLENA..PSX..HOODO..LEV.Y280.CHRGE.RAYZZ3.KTPA/0932` |
| lat/lon | 2,180 | 13% | `33.265, -113.1` (from DMS via ARTCC radar) |
| eta | 1,949 | 12% | `2026-04-05T09:31:29Z` |
| gufi | varies | ~63% | `KO126571G9` (FAA globally unique flight ID) |

**Note:** Position, altitude, speed, and ETA are only present on flights with recent track updates. Most flights have origin/destination/ETD from their filed plan even without track data. Coverage improves as more track messages arrive.

### Flight Plan Example (`GET /api/swim/flights/UAL649`)
...
```json
{
  ...
  "altitude": "330C",
  "speed": "480",
  "route": "KSFO./.EHF128063..BLH.J169.TFD.J50.ELP..KOBLE..DLF..KINEY..SLENA..PSX..HOODO..LEV.Y280.CHRGE.RAYZZ3.KTPA/0932",
  "flight_status": "ACTIVE",
  "etd": "2026-04-05T04:20:00Z",
  "eta": "2026-04-05T09:31:29Z",
  "lat": 33.265,
  "lon": -113.1,
  "reported_alt": "330C"
}
...
**Route decoded:** KSFO departure → EHF (Edwards VOR radial 128 at 63nm) → BLH (Blythe) → J169 (Jet route) → TFD → J50 → ELP (El Paso) → KOBLE → DLF (Del Rio) → KINEY → SLENA → PSX (Palacios) → HOODO → LEV (Lakeville) → Y280 (RNAV route) → CHRGE → RAYZZ3 STAR → KTPA arrival at 0932z.

### Airport Configurations (`GET /api/swim/airports`)

42 airports with real-time data. Examples:

| Airport | Runways (Arr) | Runways (Dep) | Arr Rate | Dep Rate | Weather |
|---------|---------------|---------------|----------|----------|---------|
| DFW | 36L/35C/35R/31R | 36R/35L | 124/hr | 102/hr | VMC |
| DEN | 16L/16R/17R | — | 110/hr | — | VMC |
| SLC | 16R/16L/17 | 16R/16L/17 | 105/hr | 75/hr | VMC |
| ORD | 27R/27C/28C | 22L/28R/27L | 100/hr | 100/hr | VMC |
| ATL | — | 8R/9L | — | 118/hr | LVMC |
| MCO | 18R/17L | — | 80/hr | — | VMC |
| CLT | 18R/18L | 18C/18L | 72/hr | 69/hr | VMC |
| LAX | 25L/24R | 25R/24L | 68/hr | 68/hr | VMC |
| IAH | 26R/26L | 15R/L | 56/hr | 80/hr | LVMC |
| JFK | ILS 04R/04L | 04L | 48/hr | 34/hr | VMC |
| SAT | 4.0 | 4.0 | 20/hr | 20/hr | IMC |
| HOU | 4.0 | 31L | 28/hr | 28/hr | LVMC |

**Weather conditions observed:** VMC (38 airports), LVMC (3: ATL, IAH, HOU), IMC (1: SAT).

### Flow Events (`GET /api/swim/flow`)

23,206 flow events captured. Distribution:

| Type | Count | Description | Example |
|------|-------|-------------|---------|
| TMI_LIST | 18,988 | Flights affected by Traffic Management Initiatives | Airports: ATL, MYNN, IND, RFD, HFY |
| RSTR | 3,381 | Miles-in-trail, departure restrictions | MIT at waypoints, staffing constraints |
| FXA | 406 | Flow Constrained Areas with geometry | Geographic polygons with lat/lon, FL bounds |
| GADV | 384 | ATCSCC general advisories | Reroutes, FCA declarations with full route text |
| RAPT | 32 | Route Availability Planning | (new type — not in original parser) |
| GDP | 6 | Ground Delay Programs | Active at 6 airports |
| REROUTE | 5 | Reroute advisories | Affected routes with alternate routing |
| APTC | 3 | Airport configuration changes | Runway changes (stored in airport_configs) |
| TMI_UPDATE | 1 | TMI update | (new type) |

### Terminal Weather (`GET /api/swim/weather`)

31,897 events from 88 ITWS sites. Real alerts observed:

| Type | Count | Severity | Sites |
|------|-------|----------|-------|
| Tornado | 6,919 | CRITICAL | CLE, CVG, CMH, MCO, DTW, C90, D01, D10, SDF |
| Microburst | 2,583 | CRITICAL | LAS, MIA, SDF, T75, C90, ORD |
| Windshear | 2,202 | HIGH | N90, CLT, D10, C90, SJU, ORD |
| Gust Front | 2,000 | HIGH | SJU, SDF, T75, C90, CVG, ORD |
| Precipitation | 6,386 | LOW | All sites |
| Hazard Text | 2,458 | MEDIUM | Various TRACON areas |
| Storm Motion | 1,553 | LOW | Various |

**Real alert example (ORD):**
- CRITICAL: Microburst ATIS Product at ORD, valid til 05:54z
- HIGH: Wind Shear ATIS Product at ORD, valid til 05:54z
- HIGH: Gust Front ETI Product at ORD, valid til 05:55z

### NOTAM Coverage (`GET /api/swim/notams/airports`)

739 NOTAMs, top affected airports:

| Airport | Count | RWY | TWY | APRON | AIRSPACE |
|---------|-------|-----|-----|-------|----------|
| LGA | 32 | 1 | 6 | 0 | 0 |
| FAI | 16 | 0 | 4 | 1 | 0 |
| DTW | 12 | 2 | 1 | 1 | 0 |
...
### Surface Events (`GET /api/swim/surface`)

59,319 events from 133 airports. Currently parsing:
- **TAIS** (16,723): TRACON radar tracks with callsign and TRACON ID (N90, SCT, D10, C90, etc.)
- **UNKNOWN** (42,596): Messages that need parser refinement — service detection from XML structure not yet matching all STDDS sub-services

**OOOI events** (`GET /api/swim/oooi`): Currently empty — SMES surface movement events need parser tuning for the actual STDDS XML format. The TAIS terminal tracks are flowing but SMES/TDES/APDS sub-service messages aren't being classified yet.

### Identified Gaps & Parser Issues

| Issue | Feed | Impact | Fix |
|-------|------|--------|-----|
| STDDS 42k "UNKNOWN" events | STDDS | 72% of surface data unclassified | Need to dump actual STDDS XML and update parser for real message structure |
| OOOI events empty | STDDS/SMES | No gate/taxi tracking yet | SMES messages not being detected — likely different XML structure than expected |
| Flow events mostly TMI_LIST | TFMS | TMI_LIST floods flow_events table (82%) | Consider filtering TMI_LIST to a separate table or reducing storage |
| RAPT, TMI_UPDATE types | TFMS | New flow types not in classifier | Add to `inferEventType` in tfms-parser |
| Route coverage 14% | TFMS+SFDPS | Most flights have origin/dest but not full route | Routes come from AH_FIXM/FH_FIXM (flight plan messages), not track updates. Coverage improves over time as more plan messages arrive |
| TFRs always 0 | FNS | TFR detection may need refinement | FNS AIRSPACE keyword NOTAMs (8 exist) should map to TFRs — check parser isTfr logic |
| Position coverage 13% | TFMS | Only track updates have lat/lon | Expected — most messages are plan filings without position. Live position comes from track messages which are the most frequent type |

### Data Freshness

After only 6 minutes of collection:
- **TFMS route auto-population**: 14,528 routes from TFMS vs 1,299 from ADSBdb — TFMS provides **11x more routes** than the previous ADSBdb lookup approach
- **Airport configs**: Real-time data for 42 major airports including weather conditions
- **Weather**: Active tornado, microburst, and windshear alerts across 88 sites
- **Flight tracking**: 535 actively tracked flights with position updates
