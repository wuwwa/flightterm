...
function DRow({ label, value, colorClass = 'text-fg' }) {
  return (
    <div className="flex justify-between py-0.5 px-2.5 gap-2 border-b border-white/3">
      ...
      <span className={clsx('text-right text-[11px] break-all', colorClass)}>
        {value ?? '—'}
      </span>
    </div>
  )
...
export default function DetailPanel({
  ...
  const handleAeroQuery = async () => {
    if (!flight || aeroLoading || !flight.callsign || flight.callsign === '—')
      return
  ...
  return (
    <div
      className={clsx(
        'bg-bg1 flex flex-col min-h-0',
        !mobile && 'border-l border-border overflow-y-auto'
      )}
    >
      {/* sticky header */}
      {!mobile && (
        <div className="bg-bg2 border-b border-border py-0.5 px-2.5 flex justify-between items-center text-[11px] text-fg3 sticky top-0 z-1 shrink-0">
          ...
          {flight && (
            <button
              className="bg-transparent border-none text-fg3 text-[11px] cursor-pointer py-0.5 px-1"
              onClick={onClose}
            >
              ✕
            </button>
      ...
      {!flight && (
        <div className="py-1.5 px-2.5 text-[10px] text-fg3">
      ...
      <DRow
        label="callsign"
        value={
          flight ? flight.callsign + (flight.mil ? ' · military' : '') : null
        }
        colorClass="text-ylw"
      />
      <DRow
      ...
      <DRow
        label="squawk"
        value={flight ? squawkLabel(flight.squawk) : null}
        colorClass={flight ? squawkColor(flight.squawk) : 'text-fg3'}
      />
      <DRow
        label="status"
        value={flight ? (flight.grounded ? 'ground' : 'airborne') : null}
        colorClass={flight?.grounded ? 'text-ylw' : 'text-grn'}
      />
      <DRow label="country" value={flight?.country} colorClass="text-fg3" />
      <DRow
        label="mach"
        value={flight?.mach != null ? `M${flight.mach}` : null}
        colorClass="text-acc"
      />
      <DRow
        label="IAS"
        value={flight?.ias != null ? `${flight.ias} kt` : null}
        colorClass="text-fg3"
      />

      {/* adsb.fi telemetry */}
      <Section
        title="adsb.fi telemetry"
        srcTag={{ label: 'adsb.fi', colorClass: 'text-cyn' }}
      />
      <DRow label="emergency" value={adsbfi?.emergency} colorClass="text-red" />
      <DRow
        label="vert rate"
        value={
          adsbfi?.baroRate != null
            ? `${adsbfi.baroRate > 0 ? '+' : ''}${adsbfi.baroRate} ft/min`
            : null
        }
        colorClass={
          adsbfi?.baroRate != null && Math.abs(adsbfi.baroRate) > 2000
            ? 'text-ylw'
            : 'text-fg'
        }
      />
      <DRow
        label="MCP alt"
        value={adsbfi?.navAlt != null ? `${adsbfi.navAlt} ft` : null}
        colorClass="text-cyn"
      />
      <DRow
        label="MCP hdg"
        value={adsbfi?.navHdg != null ? `${adsbfi.navHdg}°` : null}
        colorClass="text-fg3"
      />
      ...
      <DRow
        ...
        value={
          adsbfi?.type
            ? `${adsbfi.type}${adsbfi.typeDesc ? ` — ${adsbfi.typeDesc}` : ''}`
            : null
        }
      />
      ...
      <DRow
        label="IAS"
        value={apl?.ias != null ? `${apl.ias} kt` : null}
        colorClass="text-cyn"
      />
      <DRow
        label="TAS"
        value={apl?.tas != null ? `${apl.tas} kt` : null}
        colorClass="text-cyn"
      />
      <DRow
        label="mach"
        value={apl?.mach != null ? `M${apl.mach}` : null}
        colorClass="text-acc"
      />
      <DRow
        label="roll"
        value={
          apl?.roll != null ? `${apl.roll > 0 ? '+' : ''}${apl.roll}°` : null
        }
        colorClass={
          apl?.roll != null && Math.abs(apl.roll) > 25 ? 'text-ylw' : 'text-fg'
        }
      />
      <DRow
        label="turn rate"
        value={
          apl?.trackRate != null
            ? `${apl.trackRate > 0 ? '+' : ''}${apl.trackRate}°/s`
            : null
        }
        colorClass="text-fg3"
      />
      <DRow
        label="MCP alt"
        value={apl?.navAltMcp != null ? `${apl.navAltMcp} ft` : null}
        colorClass="text-cyn"
      />
      <DRow
        label="FMS alt"
        value={apl?.navAltFms != null ? `${apl.navAltFms} ft` : null}
        colorClass="text-cyn"
      />
      <DRow
        label="sel heading"
        value={apl?.navHeading != null ? `${apl.navHeading}°` : null}
        colorClass="text-fg3"
      />
      <DRow
        label="nav modes"
        value={apl?.navModes?.length > 0 ? apl.navModes.join(', ') : null}
        colorClass="text-grn"
      />
      <DRow
        label="wind"
      ...
      <DRow
        label="OAT"
        value={apl?.oat != null ? `${apl.oat}°C` : null}
        colorClass="text-fg3"
      />
      ...
      {flightroute ? (
        <>
          ...
          <DRow
            ...
            value={
              flightroute.origin
                ? `${flightroute.origin.icao_code} ${flightroute.origin.name}`
                : null
            }
            colorClass="text-acc"
          />
          ...
          <DRow
            ...
            value={
              flightroute.destination
                ? `${flightroute.destination.icao_code} ${flightroute.destination.name}`
                : null
            }
            colorClass="text-acc"
          />
      ...
      <div className="px-2.5 py-1 text-[9px] text-fg3 border-b border-white/3 leading-relaxed">
        schedule, delays, flight status & progress
      </div>
      {!flight ? (
        <>
          <DRow label="ident" />
        ...
        <DRow
          label="status"
          value={`cap reached ($${aeroSpend.total_spend.toFixed(
            2
          )} / $${aeroSpend.cap.toFixed(2)})`}
          colorClass="text-red"
        />
      ) : aeroData ? (
        <>
          <div className="bg-bg2 py-1.5 px-2.5 flex items-center justify-between border-b border-border">
            <div className="flex items-center gap-1.5 text-[12px]">
              <span className="text-acc font-bold">
                {aeroData.origin?.code_icao || aeroData.origin?.code || '???'}
              </span>
              <span className="text-fg3">→</span>
              <span className="text-acc font-bold">
                {aeroData.destination?.code_icao ||
                  aeroData.destination?.code ||
                  '???'}
              </span>
            </div>
            <span
              className={clsx(
                ...
                aeroData.status === 'En Route'
                  ...
                  ? 'border-ylw text-ylw'
                  : aeroData.status?.includes('Arrived') ||
                    aeroData.status?.includes('Landed')
                  ? 'border-acc text-acc'
            ...
            <div className="px-2.5 py-1.5 border-b border-white/3">
              ...
              <div className="h-1 bg-bg rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, aeroData.progress_percent)}%`,
                  }}
                />
              </div>
            </div>
          ...
          <div className="py-0.5 px-2.5 text-[9px] text-fg3 bg-bg/50 border-t border-white/3 mt-0.5 tracking-wider">
            DEPARTURE
          </div>
          <DRow
            label="scheduled"
          ...
          <DRow
            ...
            colorClass={
              aeroData.actual_out || aeroData.actual_off
                ? 'text-grn'
                : 'text-fg3'
            }
          />
          ...
          <DRow
            ...
            colorClass={
              aeroData.actual_in || aeroData.actual_on ? 'text-grn' : 'text-fg3'
            }
          />
          <div className="py-0.5 px-2.5 text-[9px] text-fg3 bg-bg/50 border-t border-white/3 mt-0.5 tracking-wider">
            FILED
